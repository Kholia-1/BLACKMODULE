...
def normalize_sanction_item_lengths(item: dict) -> dict:
    ...
    item["lieu_naissance"] = safe_text(item.get("lieu_naissance"), 255)
    item["nationalite"] = safe_text(item.get("nationalite"), 255)
    item["pays"] = safe_text(item.get("pays"), 100)
    item["num_passeport"] = safe_text(item.get("num_passeport"), 100)
    item["autres_documents"] = safe_text(item.get("autres_documents"), 1000)
...
def import_ofac_sdn_xml(
    ...
    for item in entries:
        ...
        if existing_entry:
            ...
            existing_entry.lieu_naissance = item.get("lieu_naissance")
            ...
            existing_entry.autres_documents = item.get("autres_documents")
